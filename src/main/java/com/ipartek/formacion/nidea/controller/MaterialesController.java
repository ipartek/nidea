...
public class MaterialesController extends HttpServlet {
	...
	private static final String VIEW_MATERIALES = "materiales.jsp";
	...
	public static final int OP_MOSTRAR_MATERIALES = 3;
	...
	private void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			switch (op) {
			case OP_MOSTRAR_MATERIALES:
				listarMaterialesUsuario(request);
				break;
	...
	private void listar(HttpServletRequest request) {
		...
		request.setAttribute("materiales", materiales);
		dispatcher = request.getRequestDispatcher(VIEW_MATERIALES);
	}
	
	private void listarMaterialesUsuario(HttpServletRequest request) {
		ArrayList<Material> materiales = new ArrayList<Material>();
		usuario = new Usuario();
		HttpSession session = request.getSession();
		usuario = (Usuario)session.getAttribute("usuario");
		
		materiales = daoMaterial.getByUser(usuario.getId());
		request.setAttribute("materiales", materiales);
		dispatcher = request.getRequestDispatcher(VIEW_INDEX);
	}
