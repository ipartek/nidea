...
public class UsuarioDAO implements Persistible<Usuario> {
	...
	public ArrayList<Usuario> getAll() {
		...
		String sql = "SELECT `id`, `nombre` FROM usuario ORDER BY usuario.id DESC LIMIT 500";
	...
	public List<Usuario> getAllApiByName(String nombre) {
		ArrayList<Usuario> lista = new ArrayList<Usuario>();
		String sql = "SELECT id , nombre FROM usuario  WHERE nombre LIKE ? ORDER BY nombre ASC LIMIT 1000;";
		try (Connection con = ConnectionManager.getConnection(); PreparedStatement pst = con.prepareStatement(sql);) {
